...
def extract_attention_values(data_dict, cls_):
    ...
    for e in entries:
        ...
        if first_occurance and rep_num != 1:
            if rep_num is not None:
                continue

        if not first_occurance and rep_num == 1:
            if rep_num is not None:
                continue
...
def plot_attention_grid(tp_posmap, fp_posmap, oth_posmap, n_layers, n_heads,
                        savepath=f"attention_grid_fo_{first_occurance}.pdf", x_min=9, x_max=141):
    fig_w = n_heads * 4
    fig_h = n_layers * 3
...
def plot_position_histograms(tp_data, fp_data, oth_data, bins=32):
    ...
    plt.tight_layout()
    plt.savefig(f"hist_tp_fp_other_fo_{first_occurance}.png", dpi=140)
    plt.show()
    
    
def plot_token_class_composition(tp_data, fp_data, oth_data, n_layers,
                                 n_heads, bin_size=2,
                                 savepath=f"token_class_composition_fo_{first_occurance}.png"):
...
def plot_overall_attention_strength(tp_posmap, fp_posmap, oth_posmap,
                                    ...
                                    savepath=f"overall_attention_strength_fo_{first_occurance}.png"):
    """
...
plot_attention_grid(tp_posmap, fp_posmap, oth_posmap, n_layers, n_heads)
...
plot_avg_attention_over_layers(tp_posmap, fp_posmap, oth_posmap,
                               ...
                               savepath=f"avg_attention_L5to19_fo_{first_occurance}.png")

plot_overall_attention_strength(tp_posmap, fp_posmap, oth_posmap,
                                st_layer=5, end_layer=19,
                                n_heads=32,
                                savepath=f"overall_attention_strength_L5to19_fo_{first_occurance}.png")