...
class CHAIR(object):
    ...
    def compute_chair(self, cap_file, image_id_key, caption_key):
        ...
        for i in tqdm.trange(len(caps)):
            ...
            # Get all words in the caption, as well as corresponding node words
            ...
            # Initialize metrics for this caption
            cap_dict['metrics'] = {
                'CHAIRs': 0,
                'CHAIRi': 0,
                'Recall': 0,
                'Precision': 0,  # Add Precision metric
                'F1': 0,
                'Len': 0,
            }
            ...
            pred_objects = set(node_words)  # all predicted unique objects in caption
            ...
            # Calculate Precision for this caption
            correct_pred_objects = pred_objects.intersection(gt_objects)
            if len(pred_objects) > 0:
                cap_dict['metrics']['Precision'] = len(correct_pred_objects) / len(pred_objects)

            # Update overall counters for Precision
            num_correct_pred_objects += len(correct_pred_objects)
            num_pred_objects += len(pred_objects)

            # Add the result for this caption to the output
        ...
        precision = num_correct_pred_objects / num_pred_objects if num_pred_objects > 0 else 0.0
        f1 = (2 * precision * recall)/(precision + recall)
        avg_len = (0.01 * len_caps / num_caps)

        # Add overall metrics to output
        output['overall_metrics'] = {
            'CHAIRs': chair_s,
            'CHAIRi': chair_i,
            'Recall': recall,
            'Precision': precision,  # Add overall Precision
            'f1': f1,
            'Len': avg_len,
        }
