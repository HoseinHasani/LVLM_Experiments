...
exp_name = f"exp__dynamicthresh_trainb_{int(balanced_train)}_testb_{int(balanced_test)}_zero_{zero_class}{sfx}"
...
pos_min, pos_max = min_position, max_position
...
def smooth_dict(d, win=3):
    ...
    for i in range(win, len(vals) - win):
        smoothed[i] = vals[i - win:i + win + 1].mean()
    return dict(zip(keys, smoothed))

optimal_thresholds = smooth_dict(optimal_thresholds, win=2)
...
def denormalize_position_minmax(
    pos_norm,
    pos_norm_min,
    pos_norm_max,
    pos_min=5,
...
for epoch in range(n_epochs):
    ...
    with torch.no_grad():
        for xb, yb in test_loader:
            ...
            pos_norm = xb[:, -1].cpu().numpy()

            pos_real = denormalize_position_minmax(
                pos_norm,
                pos_norm_min,
                pos_norm_max,
                min_position,
            ...
            pos_real = np.round(pos_real).astype(int)
            pos_real = np.clip(pos_real, min_position, max_position)

            preds = []
            for pr, p in zip(probs, pos_real):
                thresh = optimal_thresholds.get(int(p), 0.5)
                preds.append(1 if pr > thresh else 0)
            
            preds = np.array(preds)

            correct += (preds == yb.cpu().long()).sum().item()
...
with torch.no_grad():
    for xb, yb in test_loader:
        ...
        pos_real = (
            (pos_norm - pos_norm_min)
            / (pos_norm_max - pos_norm_min)
        ) * (max_position - min_position) + min_position

        pos_real = np.round(pos_real).astype(int)
        pos_real = np.clip(pos_real, min_position, max_position)

        for pr, p, yt in zip(probs, pos_real, yb.numpy()):
            thresh = optimal_thresholds.get(int(p), 0.5)
            y_pred.append(1 if pr > thresh else 0)
            y_probs.append(pr)
            y_true.append(yt)
