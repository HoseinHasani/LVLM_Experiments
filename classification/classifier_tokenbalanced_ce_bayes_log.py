...
class BayesianMLPClassifier(nn.Module):
    def __init__(
        ...
        # Register priors as buffer (moves with device, no gradients)
    ...
    def decode_positions(self, x):
        """
        Decode real token positions from normalized last feature.
        """
        pos_norm = x[:, -1]
    ...
    def forward(self, x, return_bayesian=True):
        ...
        raw_posteriors = torch.exp(raw_log_probs)
        ...
        pos_idx = pos_real - self.min_position  # index into prior table

        # Fetch log-priors
        log_prior = self.log_priors[pos_idx]  # [B, 2]

        # Bayesian correction in log-space
        bayes_log_probs = raw_log_probs + log_prior
        bayes_log_probs = bayes_log_probs - torch.logsumexp(
            bayes_log_probs, dim=1, keepdim=True
        )

        bayes_posteriors = torch.exp(bayes_log_probs)
